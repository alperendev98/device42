...
export default class Setting extends Component {
	...
	state = {
		all: false,
		instances: [], // initial instance data
	...
	deleteInstancePermanently() {
		...
		let indexOfDeleteInstance = instances.indexOf(selectedInstance);
		instances.splice(indexOfDeleteInstance, 1);
		this.refs.deleteConfirmationDialog.close();
		this.setState({ loading: true });
		let self = this;
		setTimeout(() => {
			self.setState({ loading: false, instances, selectedInstance: null }, localStorage.setItem('instances', JSON.stringify(instances)));
	...
	addNewInstance() {
		...
		if (name !== '' && address !== '' && username !== '' && password !== '') {
			...
			this.setState({ addNewInstanceModal: false, loading: true });
			let self = this;
			setTimeout(() => {
				self.setState({ loading: false, instances }, localStorage.setItem('instances', JSON.stringify(instances)));
				NotificationManager.success('Instance Added!');
			}, 1000);
		}
	}
