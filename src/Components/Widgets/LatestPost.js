...
export default class LatestPost extends Component {
   ...
   */
	...
	render() {
		const { editPostModal, addNewPostForm, editPost, addNewPostDetails } = this.state;
		return (
			<div className="blog-list-wrap">
							...
							<ListItem key={key} button className="post-item align-items-center justify-content-between py-25">
								<div className="post-content d-flex">
									...
									<div className="post-info">
										<h5><a href={data.title}>{data.title}</a></h5>
								...
								<div className="d-flex hover-action">
									<Button className="btn-success text-white m-5"  mini onClick={() => this.onEditPost(data)}>
										<i className="zmdi zmdi-edit"></i>
									</Button>
									<Button className="btn-danger text-white m-5"  mini onClick={(e) => this.onDeletePost(e, data)}>
				...
				<RctCardFooter customClasses="d-flex justify-content-between align-items-center">
					<Button  color="primary" className="text-white" onClick={() => this.addNewPost()}>
				...
				{editPostModal &&
					...
					>
						...
						<ModalFooter>
							{addNewPostForm ?
								<div>
									<Button  color="primary" className="text-white" onClick={() => this.onSubmitAddNewPostForm()}><IntlMessages id="button.add" /></Button>{' '}
									<Button  className="btn-danger text-white" onClick={this.toggleEditPostModal}><IntlMessages id="button.cancel" /></Button>
								</div>
								: <div><Button  color="primary" className="text-white" onClick={() => this.onSubmitPostEditDetailForm()}><IntlMessages id="button.update" /></Button>{' '}
									<Button  className="btn-danger text-white" onClick={this.toggleEditPostModal}><IntlMessages id="button.cancel" /></Button></div>
							}
						</ModalFooter>
					</Modal>
				}
